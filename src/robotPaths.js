...
class RobotPaths {
  ...
  solve() {
    let output = 0;
    const entireBoard = this.board.board;
    const boardSize = entireBoard.length;
    // console.log("aaaaa", this.board.hasBeenVisited(0, 0));
    ...
    const movement = (rows, columns) => {
      ...
      if (this.board.hasBeenVisited(rows, columns) === true) {
        return;
      } else {
